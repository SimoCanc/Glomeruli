...
formati_ammessi = ['jpg', 'png'] # Possibili formati delle immagini: jpg, png, ...
FORMAT = 'jpg' 
if FORMAT not in formati_ammessi: raise Exception('ATTENZIONE: Formato immagini sconosciuto!!!')
imgs_format = '.' + FORMAT

i = 0
for _, _, filenames in os.walk(path):
    for filename in filenames:
        if filename.endswith(imgs_format) and i < dim_test:
...
def sort_imgs_list(paths_to_imgs):
    ...
    foo_imgs_paths = [elem + imgs_format for elem in foo_imgs_paths]
...
def concat_images(input_folder, output_folder, imgs_resize=None):
    ...
    imgs_paths = [os.path.join(input_folder, f) 
                   for f in os.listdir(input_folder) if f.endswith(imgs_format)]
...
def concat_images(input_folder, output_folder, imgs_resize=None):
    ...
    # Salvataggio griglia
    if FORMAT == 'jpg':
        image.save(os.path.join(output_folder, 'canvas' + imgs_format), 'JPEG')
    image.save(os.path.join(output_folder, 'canvas' + imgs_format), 'PNG')    

concat_images('./CustomInput', './')

# Visualizza la griglia di immagini
#Image.open('canvas' + imgs_format)
